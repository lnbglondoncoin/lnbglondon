...
const [selectedToken, setSelectedToken] = useState("Ethereum");
  ...
  useEffect(() => {
    const main = async () => {
      if (selectedToken == "Ethereum" && tokenAmount !== 0) {
        setTimeout(async () => {
          let parse = ethers.utils.parseEther(tokenAmount?.toString() > 0 ? tokenAmount?.toString() : "0");
          ...
          if (parse > 0) {
            ...
            let tokenTokens = howMuch?.toString() / (+contractData?.tokenPrice / 10 ** 18);
  ...
  console.log(lnbgValue?.toString(), "lnbgValue?.toString()lnbgValue?.toString()lnbgValue?.toString()t");
  ...
  return (
    <div className="flex flex-col items-center gap-y-5 rounded-lg bg-black/50 px-5 py-6 md:px-8 md:py-9">
      ...
      <div className="flex w-full flex-col gap-3 rounded-xl bg-coal px-4 py-5">
        ...
        <div className="flex flex-wrap items-end gap-1 font-bold">
          <span className="text-4xl text-primary">${contractData?.raisedAmount}</span>
      ...
      <div className="flex w-full items-center">
        ...
        <div className="flex w-full items-center gap-2 rounded-r-xl border border-gray2 px-3 py-3 text-lg">
          ...
          <input
            type="text"
            // inputMode="numeric"
            value={Number(ethers.utils.formatEther(lnbgValue?.toString()))?.toFixed(6)}
            onChange={handleTokenChange}
            className="w-full bg-transparent text-gray2"
          />
        </div>
      ...
      <div className="grid w-full sm:grid-cols-2">
        <span className="text-sm font-semibold">1 lnbg = {ethers.utils.formatUnits(contractData?.tokenPrice, 18)}</span>
    ...
    {isConnected ? (
              ...
              <button  className="mt-10 w-full rounded-full bg-primary py-3 font-bold text-black" onClick={() => open()}> Connect Wallet </button>
            )}
